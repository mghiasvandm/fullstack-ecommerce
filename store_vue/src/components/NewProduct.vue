<template>
    <div>
      <TitrSection title="جدیدترین محصولات"/>
      <div class="new-products mb-16 relative sm:w-[95%] w-full mx-auto">
        <swiper :autoplay='{"delay": 3000,"disableOnInteraction": false,reverseDirection: true}' :breakpoints="{
          '300': {
          slidesPerView: 1,
          spaceBetween: 0,
          slidesPerGroup: 1
      ...
      '700': {
        ...
        '1165': {
          ...
          spaceBetween: 20,
          slidesPerGroup: 4
        },}" :slidesPerView="4" :slidesPerGroup="4" :navigation="true" :modules="modules"  class="mySwiper"   :pagination="{ clickable: true } " >
            <swiper-slide class="p-2 flex flex-col product-card"  v-for="newProduct in newProducts" v-bind:key="newProduct.id" v-bind:newProduct="newProduct">
                 <router-link :to="'/product/'+newProduct.category.name+'/'+newProduct.id">
                  <div style="width: 323px;" class="product-slider h-[400px] relative">
                   <img :src='"https://api.khoonevar.ir"+newProduct.get_picture1_url' class="rounded-md mt-2 w-full" style="height: 60%;width: 100% !important">
                  <div class="flex justify-between w-full items-center mb-5 ">
                     <div class="flex flex-col items-start w-full">
                      <div class="items-center justify-between w-full mt-5">
                        <p class="top-1 block mb-4 text-right mr-2" >{{ newProduct.name }}</p>
                        <div class="flex justify-between items-center absolute bottom-5 left-2 right-0  ">
                          <div class="mr-2 text-left mt-4  text-sm" style="padding: 3px 10px;display: flex;justify-content: center;align-items: center;border-radius: 19px;">
                      <p class="text-white relative top-0.5 bg-sky-600 px-3 flex items-center py-1 rounded-full" v-if="newProduct.discount_percent != 0">{{ newProduct.discount_percent }}%</p>
                     </div>
                          <div class="flex flex-col items-start ml-2">
                          <div class="flex items-center justify-end">
                         <h1 class="text-left mt-4 tx" v-if="newProduct.discount_percent != 0">{{ newProduct.price2.toLocaleString() }}</h1>
                         <h1 class="text-left mt-4 tx" v-if="newProduct.discount_percent == 0">{{ newProduct.price1.toLocaleString() }}</h1>
                         <svg class="mr-2" width="25" height="27" viewBox="0 0 25 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <p class="text-sm line-through text-gray-400" v-if="newProduct.discount_percent != 0" >{{newProduct.price1.toLocaleString()}}</p>
                        </div>
                        </div>
                      </div>
                    </div>
...
export default {
    name: 'NewProduct',
    components: {Swiper, SwiperSlide,TitrSection},
    setup() {
        return {
